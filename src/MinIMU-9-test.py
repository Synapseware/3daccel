...
ser = serial.Serial(port='COM11',baudrate=57600, timeout=10)
...
#f = open("Serial"+str(time.time())+".txt", 'wb')
f = open("Serial-data.txt", 'wb')
fs = struct.Struct('<f')
...
while 1:
    ser.write('A')
    try:
        data = fs.unpack_from(ser.read(4))[0]
        print "ROLL: " + str(data)
        roll = float(data)*grad2rad
        f.write("ROLL: " + str(roll) + " ")

        data = fs.unpack_from(ser.read(4))[0]
        pitch = float(data)*grad2rad
        print "PITCH: " + str(data)
        f.write("PITCH: " + str(pitch) + " ")

        data = fs.unpack_from(ser.read(4))[0]
        yaw = float(data)*grad2rad
        print "YAW: " + str(data)
        f.write("YAW: " + str(yaw) + "\n")
